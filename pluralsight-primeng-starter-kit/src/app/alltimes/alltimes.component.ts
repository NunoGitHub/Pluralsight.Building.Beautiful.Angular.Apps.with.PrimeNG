...
export class AlltimesComponent implements OnInit {
  ...
  loadTimes(event: LazyLoadEvent) {
    ...
    if (event.filters && event.filters["project"]) {
      ...
      query = table
        .where("project")
        .startsWithIgnoreCase(event.globalFilter)
        .or("user")
        .startsWithIgnoreCase(event.globalFilter)
        .or("category")
        .startsWithIgnoreCase(event.globalFilter);
    ...
    query = query.offset(event.first).limit(event.rows);
    ...
    this.contextMenu = [
      {
        label: "Debug",
        icon: "fa fa-bug",
        command: (event) => this.onDebug(this.selectedRows),
      },
      {
        label: "Delete",
        icon: "fa fa-close",
        command: (event) => this.onDelete(this.selectedRows),
      },
    ];
  }
